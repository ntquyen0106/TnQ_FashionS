...
export default function ResetPassword() {
  ...
  const onSubmit = async (e) => {
    ...
    if (!password || password.length < 6)
      return setMsg("Mật khẩu tối thiểu 6 ký tự");
    if (password !== confirm)
    ...
    try {
      ...
      await http.post("/auth/forgot/reset", { resetToken, newPassword: password });
    ...
    <div className={`card-narrow ${styles.wrap}`}>
      ...
      <form onSubmit={onSubmit}>
        ...
        <div className={styles.actions}>
          ...
          <Link className={styles.link} to="/login">Về đăng nhập</Link>
        </div>
